...
**Design system (UI):** `app/globals.css` defines `:root` CSS variables and Tailwind v4 `@theme inline` tokens — warm background, terracotta accent, card/border/shadow/radius scales, typography rhythm, **section spacing** (`--y-section-sm` / `--y-section` / `--y-section-lg`, `--gap-section`, exposed as `--spacing-section*` / `--spacing-gap` in `@theme`). **Catalog showroom** tokens: **`--r-showroom`**, **`--s-showroom-card`**, **`--s-showroom-card-hover`** (also **`--shadow-showroom-card*`** / **`--radius-showroom`** in `@theme`). **`@layer components`**: **`.catalog-showroom-stage`** and **`.catalog-showroom-product-shadow`** (pseudoelement glow/gradients + product drop shadow) for catalog grid + `ProductModal` hero; plus **`.page-shell`** (centered `max-w-7xl` column + horizontal padding), **`.section-block`** (vertical section padding via `py-section`), **`.section-heading`** and **`.section-intro`** (unified list/marketing titles + subtitle). Site chrome and main surfaces (home, catalog, articles, `Navbar`, `Footer`, `ContactModal`, `ProductCarousel`, `ReviewsGrid`) use this **light** system. **`/svo` and `/svo/[slug]`** remain **dark** by design (`SvoPageShell`, hero `#0a0a0a`, etc.) for existing product imagery; do not assume SVO was restyled with the global light tokens.
...
| `app/globals.css` | Tailwind v4 entry + global styles; **design tokens** in `:root` and **`@theme inline`** (warm background, terracotta accent, cards, borders, shadows, radii, type rhythm, **section spacing**; **showroom** catalog: **`--r-showroom`**, **`--s-showroom-card` / hover**, theme **`--shadow-showroom-card*`**, **`--radius-showroom`**); **`@layer components`**: **`.page-shell`**, **`.section-block`**, **`.section-heading`**, **`.section-intro`**, **`.catalog-showroom-stage`** (radial/linear gradients, vignette) + **`.catalog-showroom-product-shadow`**; no `prefers-color-scheme` theme flip — light showroom is default for non-SVO surfaces |
...
| `app/catalog/CatalogPageClient.tsx` | Client catalog UI (**light**); **showroom "mini vitrine"** cards: per-product **`article`**, top **`flex-[7]`** image stage + bottom **`flex-[3]`** text block (~**70/30**), **`catalogCardShell`** uses **`--r-showroom`**, **`--s-showroom-card*`**; inner **`.catalog-showroom-stage`** with overlays + **`.catalog-showroom-product-shadow`** on `img`; **two** on-card `button`s — **«Оставить заявку»** → `dispatchOpenContactModal()`, **«Подробнее»** → `ProductModal` (not a single full-card click target); **`ProductModal`**: hero **`.catalog-showroom-stage`**, `aspect-[3/2]`, **«Подобрать технику»** → `onBuyClick` / contact modal, secondary **«Подробнее»** dismisses dialog; `max-h-[90vh] overflow-y-auto`; `role="dialog"`, `aria-modal`, `aria-labelledby`, Escape + backdrop close |
...
| `app/components/Navbar.tsx` | Site navigation: **premium light** fixed header **`h-14`** (blur / glass), terracotta primary CTA **`CTA_LABEL` = «Подобрать технику»** → `dispatchOpenContactModal()`; **desktop**: centered links (incl. **Статьи** → `/articles`), muted phone text; **mobile**: phone icon link, compact CTA, **burger drawer** for nav |
...
*Last updated: 2026-04-25 — **Catalog showroom vitrines:** **`app/globals.css`** — **`--r-showroom`**, **`--s-showroom-card*`**, theme mirrors; **`@layer components`** **`.catalog-showroom-stage`**, **`.catalog-showroom-product-shadow`**. **`app/catalog/CatalogPageClient.tsx`** — grid cards as **`article`** with **`flex-[7]`** / **`flex-[3]`** (~70/30) split, on-card **«Оставить заявку»** (contact) + **«Подробнее»** (product modal), modal hero reuses **`.catalog-showroom-stage`**, primary **«Подобрать технику»** unchanged. Prior: **2026-04-24** orchestrator-only rule + showroom shell/section/nav; **2026-04-12** SVO notes in directory rows above.*
