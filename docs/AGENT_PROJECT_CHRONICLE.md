...
| `app/svo/[slug]/page.tsx` | SVO detail: `generateStaticParams` from `getSvoCatalogProducts()`, `generateMetadata` / `notFound` via `getSvoProductBySlug`, `revalidate = 60`; `SvoPageShell`; inner column **`max-w-7xl` below `lg`**, **`lg:mx-0 lg:max-w-none lg:px-8 xl:px-12`** (horizontal padding aligned with `/svo` list); **`lg` two-column grid** — wide left `SvoDetailHero`, right column optional description + `SvoPriceBlock` + `SvoDetailCta`; full-width section **`#svo-specs`** + `SvoSpecsRow` below the grid when any CMS spec is set |
| `app/svo/SvoDetailHeader.tsx` | Detail top bar: back link, centered **`h1`** with listing-matched typography (**`text-[11px] sm:text-xs`**, **`uppercase`**, **`tracking-[0.2em]`**), **«Ещё данные»** skip link to `#svo-specs` when specs exist (a11y-friendly) |
| `app/svo/SvoDetailHero.tsx` | **Client** (`"use client"`): `next/image` (`fill`, `sizes`, `priority`), `object-contain`; `imageAlt` from detail title; **`useId()`**-scoped SVG **linearGradient** ids (SSR-safe) for dimension rails; monolithic **`bg-[#0a0a0a]`** (no border/card); optional CMS length/height — **offset rails** right of image (vertical stroke) and below (horizontal) with **gradient-fade** strokes + caps; **L** / **H** each **one line**; label typography **`text-[10px]`** uppercase tracking aligned with `SvoSpecsRow` |
| `app/svo/SvoSpecsRow.tsx` | CMS specs as flex-wrap **`ul`**: each **`li`** is **label · value** on **one line** (**`text-[10px]`**, **`uppercase`**, **`tracking-[0.2em]`**, zinc label/value tones) |
...
| `app/svo/SvoPriceBlock.tsx` | Optional `priceRegular` / `priceDiscount`: **zinc** strikethrough + **zinc-100** emphasis (no green accent); returns null if both empty |
...
*Last updated: 2026-04-12 — **SVO detail layout:** inner shell **`lg:max-w-none`**, **`lg:px-8`**, **`xl:px-12`** (matches list); **`lg` two-column grid** (wide hero + right description/prices/CTA); **`#svo-specs`** full width below. **`SvoDetailHero`:** client, **`useId()`** gradient ids, **`#0a0a0a`** block, dimension rails with gradient strokes, L/H one line, typography aligned with specs. **`SvoDetailHeader`** title matches list **`h1`**. **`SvoPriceBlock`** / **`SvoDetailCta`:** zinc palette, no green. **`SvoSpecsRow`:** single-line **label · value**, uppercase tracking. **SVO list** (unchanged summary): `h1` / viewport / **`data-svo-measure`** / **`lg:grid-cols-3`** / **`1fr`/`1fr`/`auto`** when **≥4**; **SVO raster / alpha:** `docs/PROJECT_ADMIN.md` **Изображения СВО**. Earlier: SVO **`next/image`**; `svoProduct` / fallback **9**; `ReviewsGrid`; `homeCarouselSettings`.*
