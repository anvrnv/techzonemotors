...
| Path | Role |
|------|------|
| `app/layout.tsx` | Root layout: Montserrat font, `html` **`lang="ru"`**, `body` **`bg-background text-foreground`** (token-driven light chrome), `Navbar`, **`main`** **`flex min-h-0 flex-1 flex-col pt-[72px]`** (fixed-navbar clearance matches Navbar height **72 px**; **`min-h-0`** lets flex descendants shrink and fill the viewport), `GlobalContactModal`; metadata "TechZone Motors" |
| `app/globals.css` | Tailwind v4 entry + global styles; **design tokens** in `:root` and **`@theme inline`** — **blue accent** palette (`--accent: #2B6EF2`), semantic aliases (`--color-*`), button tokens (`--btn-*`), input/checkbox tokens, **radius scale** (`--r-xs` → `--r-pill`), **shadow scale** (`--shadow-xs/sm/md/lg`), transition tokens, typography scale (`--text-xs` … `--text-4xl`), section spacing; **showroom** catalog tokens (`--r-showroom`, `--s-showroom-card` / hover); **`@layer components`**: **`.btn-primary`**, **`.btn-secondary`**, **`.btn-ghost`**, **`.ui-input`**, **`.ui-select`**, **`.hero-container`**, **`.surface`**, **`.surface-soft`**, **`.product-media`**, **`.page-shell`**, **`.section-block`**, **`.section-heading`**, **`.section-intro`**, **`.catalog-showroom-stage`** (blue glow) + **`.catalog-showroom-product-shadow`**; `prefers-reduced-motion` and `:focus-visible` global rules; no `prefers-color-scheme` flip — light is default for non-SVO |
...
| `app/components/Navbar.tsx` | Site navigation: **premium light** fixed header **`h-[72px]`** (`bg-white/[0.88] backdrop-blur-[16px]`), blue primary CTA **`CTA_LABEL` = «Подобрать технику»** → `dispatchOpenContactModal()`; **desktop**: centered links (incl. **Статьи** → `/articles`, **Контакты** → contact modal), muted phone text; **mobile**: phone icon link, compact CTA, **overlay-drawer** (slide-in right, `z-[60]`/`z-[61]`, Escape key + body overflow lock, full-height panel with nav links incl. **Контакты**, footer CTA via `.btn-primary`) |
| `app/components/Footer.tsx` | Footer (**light** showroom styling, borders/shadows); **`page-shell`** / section spacing alignment with catalog and home |
...
| `app/components/ContactModal.tsx` | Contact form modal UI (**light** modal); `<label htmlFor>` over each field; per-field error states (`nameError`, `phoneError`) with `aria-invalid` + `aria-describedby`; native `<input type="checkbox">` (with `accentColor` token) instead of button-role checkboxes; submit button uses **`.btn-primary w-full`** class (blue) |
...
| `app/components/Tabs.tsx` | **New.** Accessible tab strip; props: `tabs: TabItem[]` (`id`, `label`, optional `disabled`), `activeId`, `onChange`, optional **`idPrefix`** (string; scopes generated `id="…-tab-{id}"` and `aria-controls="…-panel-{id}"` for ARIA panel wiring when multiple instances coexist on a page); WAI-ARIA `role="tablist/tab"` with `aria-selected`, `aria-controls`, `tabIndex` roving; keyboard navigation: ArrowLeft/ArrowRight/Home/End; active tab underline via `--color-primary` |
...
*Last updated: 2026-05-07 — **Блок 2 UI — Stages 5–7 (Home, Catalog, Modal):** `app/HomeClient.tsx` — полная переработка: удалён `ProductCarousel`, добавлены разделы **Hero** (два столбца, trust-point список, два CTA — `dispatchOpenContactModal()` и `router.push("/catalog")`), **Popular Models** grid (до 6 продуктов из `products` prop), **Trust/Benefits** (5 tile с inline SVG, `--color-primary-soft` bg), **ReviewsGrid** (без изменений), **Final CTA block**, `Footer`; используются `.hero-container`, `.product-media`, `.section-block`, `.section-heading`, `.btn-primary`, `.btn-secondary`. `app/catalog/CatalogPageClient.tsx` — добавлен `ProductCardSkeleton` (animate-pulse, не рендерится в дефолтном пути), empty state (SVG + текст + CTA), `Breadcrumbs` наверху страницы; карточка «Оставить заявку» использует `.btn-primary`; **`ProductModal` полностью переработан (Stage 7):** верхняя зона `.catalog-showroom-stage`, зона контента Title+Price, `Tabs` (Описание/Доставка/Гарантия, связаны через `idPrefix`/ARIA panel), trust block с SVG-иконками, кнопки CTA; body scroll lock в `useEffect`, полный keyboard focus trap, `triggerRef: useRef<HTMLButtonElement | null>` возврат фокуса на trigger через `handleClose()`, Escape + backdrop close, `role="dialog" aria-modal aria-labelledby`, close button внутри dialog; на мобильном — bottom-sheet (`items-end`), на `sm+` — по центру. `app/components/Tabs.tsx` — добавлен опциональный prop **`idPrefix`** для ARIA panel wiring. Prior: **2026-05-07** Блок 1 (синяя палитра, design tokens, Breadcrumbs, Tabs, Navbar 72 px); **2026-04-25** catalog showroom vitrines; **2026-04-24** orchestrator-only rule + showroom shell/section/nav; **2026-04-12** SVO notes.*
