...
| `app/svo/SvoPageClient.tsx` | Server UI for `/svo` list: `SvoPageShell`; neutral pluralized model count; **~900px min-height** first-viewport tuning (header margins/gaps, smaller **`h1` on `lg`**) so **2×3** tiles fit; **`lg`** image shell **`min-h`** via **`clamp`** + **`vh`** (~**14rem** floor, ~**18rem** cap; taller band when **`products.length === 1`**); single product → **`lg:col-span-3`** + **`max-w-4xl mx-auto`**; grid of `Link` → `/svo/[slug]` with `next/image` (`fill`, `sizes`); caption **`text-center`**: **bold uppercase** brand + **` / `** + normal-case **model**, else **`svoDisplayTitle()`**; local **`svoTileAccessibleText()`** sets **`Image` `alt`** and **`Link` `aria-label`** to match caption; no badge, modal, prices, or descriptions on tiles |
...
3. **SVO (`/svo`, `/svo/[slug]`):** `getSvoCatalogProducts()` in `lib/svo-products.ts` → published-only `svoProductsQuery` (no drafts) or **9-item** `lib/svo-products-fallback.ts` with slugs. **List:** `app/svo/page.tsx` (ISR `revalidate = 60`) → `SvoPageClient` (server) inside `SvoPageShell`: **~900px** first-viewport header/`h1`/gap tuning for **2×3** tiles; **`lg`** tile image height **`min-h`** **`clamp`** + **`vh`** (~**14–18rem**; taller when a single product); one item → **`lg:col-span-3`** + **`max-w-4xl mx-auto`**; each `Link` to `/svo/[slug]`; visible caption = **brand ` / ` model** (styling) or **`svoDisplayTitle()`**; **`svoTileAccessibleText()`** mirrors that string for **`alt`** / **`aria-label`**. **Detail:** `getSvoProductBySlug(slug)` → **`SvoDetailProduct`** via `svoProductBySlugQuery` (extra optional fields) or fallback; `app/svo/[slug]/page.tsx` — `generateStaticParams`, `generateMetadata`, `notFound` if missing, `revalidate = 60`; `SvoDetailHeader` / `SvoDetailHero` / conditional **`#svo-specs`** + `SvoSpecsRow` when CMS specs exist, `SvoPriceBlock`, client `SvoDetailCta`.
