...
| `app/svo/[slug]/page.tsx` | SVO detail: `generateStaticParams` from `getSvoCatalogProducts()`, `generateMetadata` / `notFound` via `getSvoProductBySlug`, `revalidate = 60`; `SvoPageShell`; inner column **`max-w-7xl` below `lg`**, **`lg:mx-0 lg:max-w-none lg:px-8 xl:px-12`**; **`lg` two-column grid** with **`lg:items-stretch`** — `SvoDetailHero` left, right column **`lg:flex-1`** + optional description + **`lg:mt-auto`** wrapper (**`SvoPriceBlock`**, **`SvoDetailCta`**); **`SvoDetailHeader`** **`product`** + **`svoTileAccessibleText`** for hero **alt**; full-width **`#svo-specs`** (no top border) + `SvoSpecsRow` when any CMS spec is set |
...
| `app/svo/SvoSpecsRow.tsx` | CMS specs as flex-wrap **`ul`**, **`justify-center`**: **·** separators **between** items only; each item **label `font-bold`** + **value `font-normal`** on one line (**`text-xs sm:text-sm`**, **`uppercase`**, **`tracking-[0.2em]`**, zinc); **no** top border in component (page section supplies spacing only) |
...
| `app/svo/SvoPriceBlock.tsx` | Optional `priceRegular` / `priceDiscount`: UI shows **one** **orange** price — **`priceDiscount` if set, else `priceRegular`** (no strikethrough, no dual-price row); **`large`** → **`text-xl sm:text-2xl`**; returns null if both empty |
...
*Last updated: 2026-04-12 — **SVO detail (second pass):** inner shell **`lg:max-w-none`**, **`lg:px-8`**, **`xl:px-12`**; **`lg` two-column grid** (taller hero ~**15–25%**, **`lg:items-stretch`**); right column **`lg:mt-auto`** groups **price + CTA** at bottom; **`SvoPriceBlock`:** single **orange** price (**discount else regular**), **no** strikethrough. **`SvoDetailHeader`:** brand/model like tiles, **larger** type. **`SvoSpecsRow`:** centered, **·** between items, **bold label** / normal value, **no** row top border in component. **`SvoDetailHero`:** **«ДЛИНА»/«ВЫСОТА»** + value one line each, visible gradient rails. **SVO list** summary: **`data-svo-measure`** / **`lg:grid-cols-3`** / **`1fr`/`1fr`/`auto`** when **≥4**; **SVO images:** `docs/PROJECT_ADMIN.md` **Изображения СВО**. Earlier: SVO **`next/image`**; `svoProduct` / fallback **9**; `ReviewsGrid`; `homeCarouselSettings`.*
