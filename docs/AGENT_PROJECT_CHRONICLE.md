...
*Last updated: 2026-04-12 — **SVO list `h1`:** typography aligned with tile captions (**`font-normal`**, **`uppercase`**, **`tracking-[0.2em]`**, **`text-[11px] sm:text-xs`**). **SVO list viewport:** `main` **`flex min-h-0 flex-1 flex-col pt-14`**, `/svo` wrapper, `SvoPageShell` **`flex min-h-0 flex-1 flex-col`**, column **`flex-1 min-h-0`**, **`lg:pt-9`**, header **`lg:mb-4`**; removed **`lg:min-h-[calc(100dvh-7.5rem)]`**. **`data-svo-measure`** + **`measure:svo-layout`**; **`lg:max-w-none`**, **`lg:grid-cols-3`**, rows **`1fr`/`1fr`/`auto`** when **≥4**. **SVO raster / alpha:** `docs/PROJECT_ADMIN.md` **Изображения СВО**. Earlier: SVO **`next/image`**; `svoProduct` / fallback **9**; `ReviewsGrid`; `homeCarouselSettings`.*
