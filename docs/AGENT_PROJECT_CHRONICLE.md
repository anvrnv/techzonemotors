...
| `app/svo/SvoPageClient.tsx` | Server UI for `/svo` list: `SvoPageShell`; neutral pluralized model count; **lg** layout + **~900px min-height** tuning for **2×3** visible tiles; header spacing; grid of `Link` to `/svo/[slug]` — `next/image` tile (`fill`, `sizes`), **`alt={svoDisplayTitle(product)}`** (duplicates visible title; `Link` keeps `aria-label`); no badge, modal, prices, or descriptions on tiles |
...
*Last updated: 2026-04-12 — SVO **block E / finish:** list + detail hero use **`next/image`** (`fill` + `sizes`; `next.config.ts` already allows `cdn.sanity.io`, local `/catalog-placeholder.jpg`); catalog tile `alt` = `svoDisplayTitle(product)`. Same day prior: SVO catalog density (`SvoPageClient`: lg, ~900px, 2×3 tiles). SVO detail: CMS optional specs + dimensions; `SvoDetailHero` (`object-contain`, dimension overlay), `SvoSpecsRow` + `#svo-specs`; seed/fallback on 1–2 slugs. Earlier 2026-04-12: `svoDisplayTitle`, `SvoPageShell`, `SvoPriceBlock` + `SvoDetailCta`, ISR/static params; `svoProduct` schema / queries / fallback **9**; 2026-04-10 — `review` / `ReviewsGrid`; 2026-04-09 — `homeCarouselSettings`.*
