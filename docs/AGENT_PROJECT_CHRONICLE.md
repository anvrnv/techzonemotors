...
| `app/components/Navbar.tsx` | Site navigation: **premium light** fixed header **`h-16 md:h-[72px]`** (responsive height; `bg-white/[0.88] backdrop-blur-[16px]`), blue primary CTA **`CTA_LABEL` = «Подобрать технику»** → `dispatchOpenContactModal()`; **desktop**: centered links (incl. **Статьи** → `/articles`, **Контакты** → contact modal), muted phone text; link hover transitions **`duration-[120ms]`**; **mobile**: phone icon link wrapped in **`Tooltip`** (`app/components/Tooltip.tsx`), compact CTA, **overlay-drawer** (slide-in right, `z-[60]`/`z-[61]`, Escape key + body overflow lock, full-height panel with nav links incl. **Контакты**, footer CTA via `.btn-primary`); burger span transitions **`duration-[180ms]`**; drawer header height matches nav (`h-16 md:h-[72px]`) |
| `app/components/Footer.tsx` | Footer (**light** showroom styling, borders/shadows); **`page-shell`** / section spacing alignment with catalog and home; social icon links sized **`h-10 w-10 flex items-center justify-center`** for accessible tap targets |
...
| `app/components/ContactModal.tsx` | Contact form modal UI (**light** modal); padding **`p-5 sm:p-8`**; uses **`modal-overlay-enter`** + **`modal-content-enter`** animation classes; `<label htmlFor>` over each field; per-field error states (`nameError`, `phoneError`) with `aria-invalid` + `aria-describedby`; native `<input type="checkbox">` (with `accentColor` token) instead of button-role checkboxes; submit button uses **`.btn-primary w-full`** class (blue); **`useToast`** hook from `Toast.tsx` — `showToast` called on success (`success` variant) / error (`error` variant); **SVG spinner** shown in submit button when `isLoading`; submit is `disabled` + `aria-disabled` while loading |
...
|| `app/components/Tooltip.tsx` | **New.** `Tooltip` component; prop `position`: `top` / `bottom` / `left` / `right`; activates on **hover**, **focus**, and **click**; closes on outside click (`mousedown` listener) |
