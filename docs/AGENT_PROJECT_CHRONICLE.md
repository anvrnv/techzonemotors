...
| `app/HomeClient.tsx` | Client home: `HomeClientProps` = `CatalogProductsProps` & `{ reviews: (ReviewData \| null)[] }`; **light** layout (`bg-background` + shared tokens), `ProductCarousel` with `onDetailsClick` → `router.push("/catalog")` («Подробнее») and `onBuyClick` → contact modal (`dispatchOpenContactModal`; carousel CTA label **«Подобрать технику»**), `ReviewsGrid` (12-slot grid from `reviews`), `Footer` |
| `app/catalog/page.tsx` | Catalog server page |
| `app/catalog/CatalogPageClient.tsx` | Client catalog UI (**light**); **showroom "mini vitrine"** cards: per-product **`article`**, top **`flex-[7]`** image stage + bottom **`flex-[3]`** text block (~**70/30**), **`catalogCardShell`**: `rounded-[24px]`, `shadow-[var(--shadow-md)]` / `hover:shadow-[var(--shadow-lg)]`, `hover:-translate-y-[3px]`; inner **`.catalog-showroom-stage`** with overlays + **`.catalog-showroom-product-shadow`** on `img`; **two** on-card `button`s — **«Оставить заявку»** → `dispatchOpenContactModal()`, **«Подробнее»** → `ProductModal` (not a single full-card click target); **`ProductModal`**: hero **`.catalog-showroom-stage`**, `aspect-[3/2]`, **«Подобрать технику»** → `onBuyClick` / contact modal, secondary **«Подробнее»** dismisses dialog; `max-h-[90vh] overflow-y-auto`; `role="dialog"`, `aria-modal`, `aria-labelledby`, Escape + backdrop close |
...
| `app/components/Tabs.tsx` | **New.** Accessible tab strip; props: `tabs: TabItem[]` (`id`, `label`, optional `disabled`), `activeId`, `onChange`; WAI-ARIA `role="tablist/tab"` with `aria-selected`, `aria-controls`, `tabIndex` roving; keyboard navigation: ArrowLeft/ArrowRight/Home/End; active tab underline via `--color-primary` |
...
*Last updated: 2026-05-07 — **Блок 1 UI-системы TechZone Motors (этапы 1–4):** `app/globals.css` — полная замена терракота-палитры на синюю (`--accent: #2B6EF2`); ~80 новых CSS-переменных (semantic `--color-*`, `--btn-*`, `--input-*`, `--checkbox-*`, radius scale `--r-xs…--r-pill`, shadow scale `--shadow-xs/sm/md/lg`, type scale, transitions); `prefers-reduced-motion` + `:focus-visible`; новые `@layer components`: `.btn-primary/secondary/ghost`, `.ui-input/select`, `.hero-container`, `.surface/surface-soft`, `.product-media`; `.catalog-showroom-stage` + `.catalog-showroom-product-shadow` переведены на синий glow. `app/components/Navbar.tsx` — высота **72 px**, glass `bg-white/[0.88] backdrop-blur-[16px]`, добавлен пункт «Контакты», мобильный drawer переработан в полноценный overlay-drawer (slide-in, z-60/61, Escape, overflow lock). `app/layout.tsx` — `pt-[72px]`, `lang="ru"`. `app/components/ContactModal.tsx` — `<label>`, per-field errors, нативные checkbox, `.btn-primary`. `app/catalog/CatalogPageClient.tsx` — `catalogCardShell`: `rounded-[24px]`, `shadow-[var(--shadow-md/lg)]`, `hover:-translate-y-[3px]`. **Новые компоненты:** `app/components/Breadcrumbs.tsx` (BreadcrumbItem, aria-current), `app/components/Tabs.tsx` (TabItem, WAI-ARIA tablist/tab, keyboard nav). Prior: **2026-04-25** catalog showroom vitrines; **2026-04-24** orchestrator-only rule + showroom shell/section/nav; **2026-04-12** SVO notes.*
