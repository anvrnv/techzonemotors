...
*Last updated: 2026-04-12 — **SVO list viewport:** `main` **`flex min-h-0 flex-1 flex-col pt-14`**, `/svo` page wrapper, `SvoPageShell` **`flex min-h-0 flex-1 flex-col`** (no **`min-h-screen`**), `SvoPageClient` column **`flex-1 min-h-0`** with **`lg:pt-9`** / header **`lg:mb-4`** — flex chain fills height under fixed navbar; removed **`lg:min-h-[calc(100dvh-7.5rem)]`** (was double-counting nav). Still: **`data-svo-measure`** + **`measure:svo-layout`** + sample dumps `docs/svo-layout-measured-*.md`; **`lg:max-w-none`**, **`lg:grid-cols-3`**, rows **`1fr`/`1fr`/`auto`** when **≥4**; tile flex + image **`flex-1 min-h-0`**. **SVO raster / alpha:** `docs/PROJECT_ADMIN.md` **Изображения СВО**. Earlier same month: SVO **`next/image`** detail/list; `svoProduct` / fallback **9**; 2026-04-10 — `ReviewsGrid`; 2026-04-09 — `homeCarouselSettings`.*
