...
| `app/components/Navbar.tsx` | Site navigation; typography aligned with SVO tile captions (**`font-normal`**, **`uppercase`**, **`tracking-[0.2em]`**, **`text-[11px] sm:text-xs`**) on logo mark **TZ**, brand text **TECHZONEMOTORS** (one word; link **`aria-label`** «TechZone Motors — на главную»), desktop + mobile nav links, phone, **Купить** CTA; center + mobile **Статьи** → `/articles` |
...
*Last updated: 2026-04-12 — **Navbar:** same caption-style typography (**`uppercase`**, **`tracking-[0.2em]`**, **`text-[11px] sm:text-xs`**); visible brand **TECHZONEMOTORS** + **`aria-label`** on home link. **SVO list `h1`:** same tile-caption typography. **SVO list viewport:** `main` **`flex min-h-0 flex-1 flex-col pt-14`**, `/svo` wrapper, `SvoPageShell` **`flex min-h-0 flex-1 flex-col`**, column **`flex-1 min-h-0`**, **`lg:pt-9`**, header **`lg:mb-4`**; removed **`lg:min-h-[calc(100dvh-7.5rem)]`**. **`data-svo-measure`** + **`measure:svo-layout`**; **`lg:max-w-none`**, **`lg:grid-cols-3`**, rows **`1fr`/`1fr`/`auto`** when **≥4**. **SVO raster / alpha:** `docs/PROJECT_ADMIN.md` **Изображения СВО**. Earlier: SVO **`next/image`**; `svoProduct` / fallback **9**; `ReviewsGrid`; `homeCarouselSettings`.*
