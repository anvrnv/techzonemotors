...
**Design system (UI):** `app/globals.css` defines `:root` CSS variables and Tailwind v4 `@theme inline` tokens — warm background, terracotta accent, card/border/shadow/radius scales, typography rhythm, **section spacing** (`--y-section-sm` / `--y-section` / `--y-section-lg`, `--gap-section`, exposed as `--spacing-section*` / `--spacing-gap` in `@theme`). **`@layer components`** adds shared layout utilities: **`.page-shell`** (centered `max-w-7xl` column + horizontal padding), **`.section-block`** (vertical section padding via `py-section`), **`.section-heading`** and **`.section-intro`** (unified list/marketing titles + subtitle). Site chrome and main surfaces (home, catalog, articles, `Navbar`, `Footer`, `ContactModal`, `ProductCarousel`, `ReviewsGrid`) use this **light** system. **`/svo` and `/svo/[slug]`** remain **dark** by design (`SvoPageShell`, hero `#0a0a0a`, etc.) for existing product imagery; do not assume SVO was restyled with the global light tokens.
...
| `app/globals.css` | Tailwind v4 entry + global styles; **design tokens** in `:root` and **`@theme inline`** (warm background, terracotta accent, cards, borders, shadows, radii, type rhythm, **section spacing** tokens); **`@layer components`**: **`.page-shell`**, **`.section-block`**, **`.section-heading`**, **`.section-intro`**; no `prefers-color-scheme` theme flip — light showroom is default for non-SVO surfaces |
...
| `app/catalog/CatalogPageClient.tsx` | Client catalog UI (**light** surfaces / modal, terracotta CTAs, borders/shadows aligned with `globals.css` tokens); **`.page-shell`** + **`.section-heading`** / intro for page chrome; `ProductModal` primary CTA **«Подобрать технику»** → `dispatchOpenContactModal()`; `aspect-[3/2]` hero + bottom gradient overlay (same pattern as home carousel), `max-h-[90vh] overflow-y-auto`; `role="dialog"`, `aria-modal`, `aria-labelledby`, Escape closes |
