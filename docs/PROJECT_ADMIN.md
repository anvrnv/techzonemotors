...
- **Страница СВО (`/svo`)** — документы `svoProduct` из Sanity; fallback — `lib/svo-products-fallback.ts`; фон раздела тёмный (см. подраздел **Изображения СВО** ниже).
- **Статьи** — публичные маршруты `/articles` (список) и `/articles/[slug]` (материал). В Sanity документ `article` (заголовок, slug, краткое описание для списка и meta description, тело Portable Text). Studio: раздел **Статьи**. Кэш страниц обновляется примерно раз в 60 с (ISR). В шапке ссылка **Статьи** (не отдельный пункт «Связаться» в центре меню — контакт через кнопку **«Подобрать технику»** в шапке / на карточках / в модальном окне товара и по телефону).
- **Отзывы на главной** — до 12 опубликованных документов `review` из Sanity (имя, текст, оценка 0–10, порядок `sortOrder`). На сайте оценка показывается звёздами как «десятка / 2» (например 10 → 5 звёзд, 7 → 3,5). Studio: раздел **Отзывы**. Если документов нет или Sanity недоступен, на главной отображается сетка из 12 пустых карточек-заглушек.
...
Раздел **Техника СВО** на сайте — [`/svo`](https://techzonemotors.ru/svo) (тёмный фон). Если вокруг мотоцикла или другой техники видна серая «шахматная» сетка, это **не ошибка вёрстки**: сетка уже **записана в пиксели файла** как имитация прозрачности.

**Единственное исправление** — заменить картинку в CMS на корректный **PNG или WebP с настоящим альфа-каналом** (документ `svoProduct` в Studio → поле фото). Диагностика: откройте исходный файл на чёрном фоне или в просмотрщике ОС — сетки быть не должна. Если она есть — пересохраните файл (например «Save for Web» / экспорт с альфой из редактора) или подготовьте в другом инструменте и снова загрузите ассет в Studio.
...
- **Карусель главной:** откройте Studio → в меню **Карусель главной** добавьте ссылки на товары (`product`) в нужном порядке — так задаётся последовательность карточек на главной (при пустом списке или сбое используется тот же набор, что у полного каталога).
